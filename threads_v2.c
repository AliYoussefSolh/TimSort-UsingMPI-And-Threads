...
void _tim_sort(int *arr, int left, int right)
{
    if (left < right)
    {
        ...
        else
        {
            ...
            pthread_t threads[2];
            ThreadData *threadData1 = (ThreadData *)malloc(sizeof(ThreadData));
            threadData1->A = arr1;
            threadData1->size = len;
            pthread_create(&threads[0], NULL, thread_handler, (void *)threadData1);
            ThreadData *threadData2 = (ThreadData *)malloc(sizeof(ThreadData));
            threadData2->A = arr2;
            threadData2->size = len2;
            pthread_create(&threads[1], NULL, thread_handler, (void *)threadData2);
            for (int i = 0; i < 2; i++)
                pthread_join(threads[i], NULL);
            
            // _tim_sort(arr, left, mid);
            // _tim_sort(arr, mid + 1, right);
            merge(arr, arr1, arr2, len, len2);
...
void *thread_handler(void *args)
{
    ...
    _tim_sort(data->A, 0, data->size - 1);
}
...
int main(int argc, char **argv)
{
    ...
    while (t--)
    {
        ...
        { // requesting data from parent process
        ...
        else if (left_child == world_size - 1) //case when process only has one child, the left child
        {
            int n1 = array_size / 2;
            int rem = array_size % 2;
            int n2 = n1 + rem;
            ...
            merge(array, arr_left_child, arr_same, n1, n2);
        }
        else
        {
        ...
        if (rank == 0)
        {
            ...
            // printf("is array sorted: %d ", is_sorted(array, array_size));
